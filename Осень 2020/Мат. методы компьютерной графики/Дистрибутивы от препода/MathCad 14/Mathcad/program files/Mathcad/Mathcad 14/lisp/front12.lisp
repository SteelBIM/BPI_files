...
;;  (setf *abort-mutex*  (GetMutex *queue*))
  ...
  (setf *engine* (Engine.MCCore12 enotify 0))
...
(defun iengine/unload ()
  (setf *notify* nil)
  (with-engine-lock
   (SetOption *engine* "disable_messages" t))
  (on-worker-thread
   (setf *queue* nil)
   (Unload *depman*)
   (setf *depman* nil)
...
(defun iengine/stop-worker ()
  (if (> *threads* 0)
      (progn
        (Efi.Symbolics.RequestInterrupt 1)
        (StopWorker *queue*)
        (EmptyQueue *queue*))))
...
(defun iengine/start-worker ()
  (if (> *threads* 0)
      (progn
        (Efi.Symbolics.RequestInterrupt 0)
        (StartWorker *queue*))))
  ...
  (lisp-action 
   ...
   (lambda (env)
     (let* ((r    (Invoke action env))
            (next (get_hash r "%next")))
       (if next
           (progn (set_hash r "%next" (hook-action next f))
                  r)
           ;; else
           (f r))))))
...
(defun handle-numeric-rhs-no-ranges (fnode tag reg tree global? lhs rhs)
  (let* ((tagged-eval (car rhs))
         ...
         (tmp         (make-temp))
         (tmp-s       (ToString tmp))
         (expr        `(let ((dummy ,plain-rhs)) 
                        (seq (numericcb 
                              ,reg ,tree 
                              (,tagged-eval dummy ,@units)) dummy)))
         (def         (if global? 'global 'define))
         (def         (set-tag def (find-tag tagged-eval))))

    (make-action fnode `(,def ,tmp ,expr) t)
    ...
    (lambda (env)
      (let ((res (Lookup env (strcat "value " tmp-s))))
        (if (has-errors res)
            (signal res)
            ;; else
            (let* ((new-rhs-val (get-result-from-dict res tmp-s)))
              (make-cast (get-rl new-rhs-val)
                         (get-units-rl new-rhs-val))))))))
  ...
  (destructuring-bind (params rhs-noparams) (get-params rhs)
    (let* ((subscripts (gather-subscripts lhs))
           ...
           (rhs1     (if unbound
                         ...
                         rhs1))
           ;; We don't want evaluation of the expression to
           ;; side effect the tree; we call the callback explicitly
           ;; below.  (This fixes bug 061115-111411.)
           (rhs1     (strip-symbolic-callbacks rhs1))
           (tmp      (make-temp))
           (tmp-s    (ToString tmp))
           (def      (if global? 'global 'define)))

      (make-action fnode `(,def ,tmp ,rhs1) t)
          ...
          (if (has-errors res)
              (if opt?
                  (progn (send-optimize-message reg "")
                         rhs)
                  ;;(list nil nil (make-eng-result nil res)))
                  (signal res)))
          ;; else
          (let* ((new-rhs-val (get-result-from-dict res tmp-s))
                 (result-rl   (get-rl new-rhs-val))
                 (new-rhs     (merge-tags rhs result-rl))
                 (optimized?  (and opt? (is-optimized rhs-noparams new-rhs))))
            (if opt?
                (send-optimize-message
                 reg (if optimized?
                         (get_ValueXML new-rhs-val)
                         ""))
                (send-symbolic-message reg tree new-rhs-val))
...
(defun strip-symbolic-callbacks (expr)
  (cond ((atom expr)                  expr)
        ((eqt (car expr) 'symboliccb) (nth 3 expr))
        (t                            (mapcar strip-symbolic-callbacks expr))))
...
(defun get-defined-vars (expr)
  (cond ((atom expr) (list expr))
        ...
        ((eqt (car expr) 'op_matcol)
         (destructuring-bind (m col) (cdr expr)
           (get-defined-vars m)))
        (t (apply append (mapcar get-defined-vars expr)))))
...
(defun make-rhs-eval (fnode tag expr)
  (let* ((nn    (nth 1 expr)) ; We have (eval (numeric (numeric ...)) ...)
    ...
    (make-action fnode `(define ,tmp ,e) t)
      ...
      (lisp-action 
       ...
       (lambda (env)
         (let ((res (Lookup env (strcat "value " tmp-s))))
           ...
           (if (has-errors res)
               (signal res)
               (let* ((val  (get-result-from-dict res tmp-s))
                      (rexp (get-rl val))
