...
// Finance Functions
   ...
   mc_cnper :: V{}->V{}->V{}->V{}                  @"[efifinance?]Efi.Finance::cnper";
   mc_crate :: V{}->V{}->V{}->V{}                  @"[efifinance?]Efi.Finance::crate";
   mc_cumint:: V{}->V{}->V{}->V{}->V{}->D{}->V{}   @"[efifinance?]Efi.Finance::cumint";
   mc_cumprn:: V{}->V{}->V{}->V{}->V{}->D{}->V{}   @"[efifinance?]Efi.Finance::cumprn";
   mc_eff   :: V{}->V{}->V{}                       @"[efifinance?]Efi.Finance::eff";
   mc_fv    :: V{}->V{}->V{}->D{}->D{}->V{}        @"[efifinance?]Efi.Finance::fv";
   mc_fvadj :: V{}->V{}->V{}                       @"[efifinance?]Efi.Finance::fvadj";
   mc_fvc   :: V{}->V{}->V{}                       @"[efifinance?]Efi.Finance::fvc";
   mc_ipmt  :: V{}->V{}->V{}->V{}->D{}->D{}->V{}   @"[efifinance?]Efi.Finance::ipmt";
   mc_irr   :: V{}->D{}->V{}                       @"[efifinance?]Efi.Finance::irr";
   mc_mirr  :: V{}->V{}->V{}->V{}                  @"[efifinance?]Efi.Finance::mirr";
   mc_nom   :: V{}->V{}->V{}                       @"[efifinance?]Efi.Finance::nom";
   mc_nper  :: V{}->V{}->V{}->D{}->D{}->V{}        @"[efifinance?]Efi.Finance::nper";
   mc_npv   :: V{}->V{}->V{}                       @"[efifinance?]Efi.Finance::npv";
   mc_pmt   :: V{}->V{}->V{}->D{}->D{}->V{}        @"[efifinance?]Efi.Finance::pmt";
   mc_ppmt  :: V{}->V{}->V{}->V{}->D{}->D{}->V{}   @"[efifinance?]Efi.Finance::ppmt";
   mc_pv    :: V{}->V{}->V{}->D{}->D{}->V{}        @"[efifinance?]Efi.Finance::pv";
   mc_rate  :: V{}->V{}->V{}->D{}->D{}->D{}->V{}   @"[efifinance?]Efi.Finance::rate";
...
//*****************************************************************
   ...
   data Token = STok String        // string
           ...
           // | T_RD_INF
              ...
              | T_PLUS | T_MINUS | T_MULT | T_DIVIDE | T_POW | T_UMINUS
	      | T_OP_PLUS | T_OP_SUB | T_OP_MULT | T_OP_DIV | T_OP_POWER | T_OP_NEGATE
              | T_CROSSPROD
   ...
   lexFloatAlternatives fmt = (lAlt (lBind (lexOneOrMoreWhile (is_digit fmt))        // digits { floatTailWithDigits }
                                  (\_ -> (lexFloatTailWithDigits fmt)))
                            (lexFloatTailSansDigits fmt));                       // nodigits { floatTailSansDigits }

   lexFloatTailWithDigits :: Format -> Lexer Int;
   lexFloatTailWithDigits fmt = (lAlt
...
//   lexFloatTailSansDigits :: Format -> Lexer Int;
//   lexFloatTailSansDigits fmt = lFail;

   // Lex a floating-point number that begins with a decimal point
   lexFloatTailSansDigits :: Format -> Lexer Int;
   lexFloatTailSansDigits fmt =
...
// Formats for parsing and emission
   ...
   freeOf var exp = case exp of {
      (Literal tok) -> case tok of { (Id name)  -> if (expr_eq var exp)
                                                   then False
                                                   else True;
                                     non_id_tok -> True;
                                   };
   ...
   betaReduce oldvar newvar exp = case exp of {
   ...
   {
     transform :: Expr -> Expr;
     transform = (betaReduce oldvar newvar);
   };
   ...
   parseWith (Parser p) s = case (p s) of {
    (ParseResult _ x) -> x;
    _                 -> error "symbolics: parse error";
   };

   parseString :: String -> Expr;
   parseString s = parseAnyString MuPad s;
...
// Operators
   ...
   lbp XML T_OP_PLUS   = 100; // + (Mathcad) = op_plus
   rbp XML T_OP_PLUS   = 101; // + (Mathcad) = op_plus
...
// Utilites
   ...
   build_matrix _             = error "efisymbolicsparser: bad matrix";

   // build a proc
   build_xproc1 [v,e] = SLam [v] e;
   build_xproc1 _     = error "efisymbolicsparser: ill-formed xproc1";

   // build a mcfloat (by picking out the expression, dropping precision for now)
   build_mcfloat [r,p,i] = make_complex r i;
   build_mcfloat [r,p]   = r;
   build_mcfloat [r]     = r;
   build_mcfloat _     = error "efisymbolicsparser: ill-formed mcfloat";

   // build a hold (by picking out the expression, dropping extraneous args)
   build_hold [x] = x;
   build_hold _     = error "efisymbolicsparser: ill-formed hold";
...
// General functions for emission
  ...
  build_notIn :: Expr -> Expr -> [Expr];
  build_notIn x y =
    case y of {
      (App (Literal T_SET) []) -> [];
      (App (Literal T_SET) (e1:emore)) ->
        ((App (Literal T_MC_NEQ) [x,e1]):(build_notIn x (App (Literal T_SET) emore)));
      _ -> [(App (Literal T_NOT) [(App (Literal T_IN) [x,y])])];
    };
...
//   been made by this point.
   ...
   emitIfThen XML ((Literal T_OTHERWISE),expr) =
    emitTagged "ml:otherwise" [] (emitExpr XML expr);

   emitIfThen XML (condition,expr) =
    emitTagged "ml:ifThen" [] (emitExprSeq XML [condition,expr]);
   ...
   emitToken XML T_OP_SUB       = emitToken XML T_MINUS;
   emitToken XML T_OP_PLUS      = emitToken XML T_PLUS;
   emitToken XML T_OP_MULT      = emitToken XML T_MULT;
   emitToken XML T_OP_DIV       = emitToken XML T_DIVIDE;
   emitToken XML T_OP_POWER     = emitToken XML T_POW;
   emitToken XML T_OP_NEGATE    = emitToken XML T_UMINUS;
...
//***********************************************************************
   ...
   display_map =
       ...
       [
        // Long names for operators defined in the op_map of lexOp ...
               ("op_add",       T_PLUS),      // +
               ("op_sub",       T_MINUS),     // -
               ("op_negate",    T_UMINUS),    // - [alternate]
               ("op_mult",      T_MULT),      // *
               ("op_div",       T_DIVIDE),    // /
               ("op_power",     T_POW),       // ^
               ("op_lt",        T_LT),        // <
               ("op_gt",        T_GT),        // >
               ("op_leq",       T_LTEQ),      // <= [composed]
               ("op_geq",       T_GTEQ),      // >= [composed]
               ("op_neq",       T_NEQ),       // <> [composed]
               ("op_eq",        T_EQ),        // =
               ("op_factorial", T_FACTORIAL), // !
               ("op_and",       T_AND),       // and
               ("op_or",        T_OR),        // or
               ("op_not",       T_NOT),       // not
               ("op_xor",       T_XOR)        // xor
   ...
   omaptok XML tok@(Id s) = case (mlookup display_map s) of {
                             Nothing   -> tok;
                             Just tok2 -> tok2;
                            };
...
// ****************************************************************
   ...
   make_sym_real = parseString;

   make_sym_imag :: String -> Expr;
   make_sym_imag = make_imag.parseString;
