...
//*
//*    $Logfile:: /MathcadClient/Trunk/efi/efisymbolics-umgd/mupad/mathcad/lib/keywords.mu $
//*   $Revision:: 125                                                                      $
//*     $Author:: kpitman                                                                  $
//*       $Date:: 2006-12-15 23:46:03-05:00                                                $
//* $NoKeywords::                                                                          $
//*
...
keywords::keyword_float_function := proc(expression, precision=hold(COMPUTED))
 begin
   if not (precision = COMPUTED) then
      checkInteger(precision);
      if (precision < 0) or (precision > 250) then
         formatError(errorBadDigits, precision);
      end_if;
   end_if;
...
keywords::keyword_explicit := proc()
 begin
   internalError("The 'explicit' keyword should not be passed to MuPad.");
 end_proc:
      ...
      then
         ...
         if isEquation(a) or isSymbol(a) then
            if x = NONE then
               ...
               formatError("var or var=val must occur only once");
            end_if;
            ...
            formatError("Bad argument to confrac");
         end_if;
      end_if;
   end_for;
   ...
   if isFloating(r) then
      ...
      if not testtype(cf, numlib::contfrac) then
         ...
         if fmt = DEFAULT or fmt = hold(mc_vector) then 
          ...
          //cf[linalg::nrows(cf),2]:=0;
            ...
            formatError("Format must be 'fraction' or 'matrix' or 'vector'.")
         end_if;
      end_if;
   else      
      ...
      if not testtype(cf, contfrac) then
         ...
         if fmt = DEFAULT or fmt = hold(mc_matrix) then 
            ...
            formatError("Format must be 'fraction' or 'matrix'.")
         end_if;
      end_if;
   end_if;
 end_proc:
 ...
 begin
   ...
   for modifier in [args(2..args(0))] do
      if modifier = hold(mc_trig) then
         ...
         end_if;
         amount := MIN;
      elif  modifier = hold(mc_none) then
         if amount <> DEFAULT then
            formatError(errorBadSyntax);
         end_if;
         amount := NONE;
         ...
         formatError(errorInvalidModifier, modifier);
      end_if;
   end_for;
   if trigsimp then
...
keywords::keyword_solve := proc(expression)
 ...
 begin
   ...
   if vars=[] then vars:=[op(expression,1)]; end_if;
   ...
   //print("expression=",expression,"vars=",vars);
   for item in vars do 
   ...
   eqn_func := unknowns -> expression(op(unknowns));
...
keywords::keyword_substitute := proc(expression)
 ...
 begin
   ...
   keyword_substitute_function(eval(expression), 
                               map(map(substitutions,toEquation),
                                   ((x)->eval(lhs(x))=eval(rhs(x)))));
 end_proc:
