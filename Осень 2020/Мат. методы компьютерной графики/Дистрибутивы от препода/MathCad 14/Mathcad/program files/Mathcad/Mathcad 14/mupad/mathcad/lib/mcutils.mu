...
//*
//*    $Logfile:: /MathcadClient/Trunk/efi/efisymbolics-umgd/mupad/mathcad/lib/mcutils.mu $
//*   $Revision:: 274                                                                     $
//*     $Author:: kpitman                                                                 $
//*       $Date:: 2007-01-03 15:30:56-05:00                                               $
//* $NoKeywords::                                                                         $
//*
...
//hold(isFloat),
  ...
  hold(optionFully),
  hold(optionGenTempVarNumber),
  hold(optionGenTempVarNumberStart),
...
mcutils::genTempVar := proc(prefix = "_TEMP_")
 begin
   text2expr(prefix.((optionGenTempVarNumber:=optionGenTempVarNumber+1)));
 end_proc:
...
mcutils::mcPartFracWorker := proc(r, v, elements=[])
 ...
 begin
   ...
   if (nv = NONE) then
      // print("mcPartFracWorker returns",res);
      res;
   else
      // print("mcPartFracWorker returning(raw)",res);
      res := mcSubs(res, [nv=ov]);
...
mcutils::initializeSolve := proc()
 begin
   ...
   optionGenTempVarNumber:=optionGenTempVarNumberStart;
 end_proc:
...
mcutils::optionGenTempVarNumberStart          := 0:
mcutils::optionGenTempVarNumber               := optionGenTempVarNumberStart:
...
mcutils::optionSimulateMTaylor                := FALSE: // mtaylor is not debugged
mcutils::optionRaw                            := FALSE:
mcutils::optionExperimentally                 := FALSE:
...
mcutils::involvesBooleans := proc(x)
 begin
   has(x,{hold(_and),   hold(_or),   hold(_not),   hold(_xor),       // MuPAD booleans
...
mcutils::mcSolve1 := proc(equations, vars)
 ...
 begin
   ...
   eqns := equations;
   if not isList(eqns) then
      eqns := [equations]; 
   end_if;
   if allTrue(eqns, (x->testtype(x,Type::PolyExpr(v)))) then
      multi := Multiple;
   end_if;
   //print("fixing up eqns=",eqns);
   //eqns := fixupProcs(eqns);
   //print("fixed up eqns=",eqns);
   ...
   while TRUE do
      ...
      if (op(msg,1)<>1028) then 
         ...
         if (isList(vars) or isSet(vars)) and nops(vars)=1 then
            vars := op(vars,1);
...
mcutils::isInequality := proc(x)
 ...
 begin
   ...
   if testtype(x,DOM_EXPR) then
      // print("isInequality descending DOM_EXPR");
...
mcutils::processSolveInequality := proc(vars, results)
 ...
 begin
   ...
   for i from 1 to nops(vars) do
      ...
      if not res1[1] then formatError(errorSymbolic); end_if;
...
mcutils::toAssumptions := proc(expression)
 local kind, arg1, arg2, result, temp, clause;
 begin
   ...
   if ( kind = hold(_equal) or kind = hold(op_eq) ) then
      if (arg2 = hold(mc_complex)) then
         result := [hold(domTypeAssumption)(arg1,hold(Type::Complex))];
      elif (arg2 = hold(mc_real)) then
         result := [hold(domTypeAssumption)(arg1,hold(Type::Real))];
      elif (arg2 = hold(mc_integer)) then
         result := [hold(domTypeAssumption)(arg1,hold(Type::Integer))];
      elif optionAllowEvenOdd and (arg2 = hold(mc_even)) then
         result := [hold(domTypeAssumption)(arg1,hold(Type::Even))];
      elif optionAllowEvenOdd and (arg2 = hold(mc_odd)) then
         result := [hold(domTypeAssumption)(arg1,hold(Type::Odd))];
         ...
         result := [hold(domTypeAssumption)(arg1,hold(Type::Interval)(op(temp)))];
      else
...
mcutils::checkKeyword := proc(keyword, validKeywords)
 ...
 begin
   for validKeyword in op(validKeywords) do
      ...
      end_if;
    end_for;
   formatError("Got %1 where one of %2 expected.", keyword, validKeywords);
 end_proc:
...
mcutils::flattenSolveResultsPiece := proc(conditions, results)
 save solveConditions;
 local c, temp;
 begin
   solveConditions := TRUE;
   temp := matrix(flattenSolveResults(results));
...
mcutils::flattenSolveResults := proc(results)
 ...
 begin
   oper := op(results,0);
   // print("flattenSolveResults",results,"oper=",oper);
   answer:=
   ...
   for i from 1 to nops(vars) do
      ...
      if not ok then
         if (domtype(set)=solvelib::cartesianPower 
            ...
            // print("Trying to get one numeric solution.");
...
mcutils::rectForm := proc(expression)
 local real_part, imag_part, result;
 begin
   ...
   real_part := mcad_Re(expression);
   // print("mcad_Re(",expression,")=",real_part);
   imag_part := mcad_Im(expression);
 ...
 begin
   if isSymbol(var) then
      if nops(more_vars) = 0 then
         collect(polynomial,var);
      else
         collect(polynomial,var,c->collectCoefficientsScalar(c,more_vars));
      end_if;
   else
 ...
 begin
   ...
   if more_vars = [] then
      ...
      polynomial := collect(polynomial,
                            tempvar,
                            c->collectCoefficientsScalar(mcSubs(c, [tempvar = expression]),more_vars));
   end_if;
   // print("collectComplicatedCoefficient returning",polynomial);
   polynomial;
 end_proc:
...
mcutils::mcSubs := proc(expression, substitutions)
 local termsubs, facsubs, divsubs, othersubs, result,
   substitution, lsub, ldiv, do_div, do_fac, substSimplify, termrule, divrule, facrule;
 begin
   substSimplify := id;
...
mcutils::checkPiecewise := proc(pw, recursively=TRUE)
 local pw1, good_cases, n_good_cases, middle_cases;
 begin
...
mcutils::adjustPiecewise := proc(pw)
 local guardian, consequent, new_guardian, new_consequent, clauses, res, res1, clause;
...
mcutils::toRewriteTarget := proc(target)
 begin
   ...
   // the tokens E and its value are possible and distinct
      ...
      // Inverse Trig
   elif (target = hold(mc_arcsin) /* or target = mc_arcsin */  ) then
      arcsin;
   elif (target = hold(mc_arccos) /* or target = mc_arccos */  ) then
      arccos;
   elif (target = hold(mc_arctan) /* or target = mc_arctan */  ) then
      arctan;
   elif (target = hold(mc_arccot) /* or target = mc_arccot */  ) then
      arccot;
...
mcutils::transformExpression := proc(expression,
 ...
 local result, expression1, result1, xnewvar, action;
 begin
 ...
 begin
   ...
   if testtype(X, "op_sqr") then
      ...
      if type(b1) in {"mcad_csgn", "mcad_sign"} then
         ...
         if is(e1,Type::Even) then
            return(1);
         elif is(e1,Type::Odd) then
            return(b1);
         else
            return(X);
         end_if;
