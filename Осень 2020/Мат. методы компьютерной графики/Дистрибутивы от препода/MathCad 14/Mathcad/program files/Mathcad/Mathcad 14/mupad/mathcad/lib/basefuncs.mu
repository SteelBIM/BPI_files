...
//*
//*    $Logfile:: /MathcadClient/Trunk/efi/efisymbolics-umgd/mupad/mathcad/lib/basefuncs.mu $
//*   $Revision:: 196                                                                       $
//*     $Author:: ltravis                                                                   $
//*       $Date:: 2007-01-04 17:27:58-05:00                                                 $
//* $NoKeywords::                                                                           $
//*
...
basefuncs::op_matcol := proc(M,c,ORIGIN=0)
 begin
   if isMatrix(M) then
      linalg::col(M, c-ORIGIN+1);
   else
      procname(args());
   end_if;
 end_proc:
...
basefuncs::mcad_eigenvecs := proc(M, side=Null)
 ...
 begin
   ...
   if (side=Null or side="R") then // omitting arg is same as passing "R"
      ...
      formatError("Side must be \"R\" (the default) or \"L\".");
   end_if;
...
basefuncs::op_rangesum := proc(f,range)
 ...
 begin
   ...
   adjustPiecewises(sum(f(var),var=fixupRange(range)));
 end_proc:
...
basefuncs::op_rangeproduct := proc(f,range)
 ...
 begin
   ...
   var:=functionArgs(f); // op(f,1);
   if (nops(var) <> 1) then internalError("Ill-formed function."); end_if;
   adjustPiecewises(product(f(var),var=fixupRange(range)));
 end_proc:
...
basefuncs::mc_integral_:=proc(ignored_tol,ignored_callback,func,lower,upper,ignored_tag)
 local main, var, action, raw_action, result, opts, mcfloating;
 begin
   ...
   end_if;
   if evalFloatNow 
// Should this additional condition do it, too, or should we leave that to int to decide?
// For now, leave it off. -kmp 25-Sep-2006
//    or stdlib::hasfloat(main)
//    or stdlib::hasfloat(lower)
//    or stdlib::hasfloat(upper)
   then
...
basefuncs::mcad_Ei_scalar := proc(optional_k, required_z)
 ...
 begin
   ...
   if args(0) = 1 then
      z := args(1);
      // In the one-arg case, compute the Cauchy Principal Value
      -Re(Ei(-z));
   else
      k := args(1);
      z := args(2);
...
basefuncs::op_solve := proc(IGNORED_opts,IGNORED_tol,IGNORED_ctol,is_minerr,IGNORED_objective,IGNORED_num_guess,IGNORED_iguess,
 ...
 local unknowns,sense_check,res,subsin,subsout,eqns,unk,rawres;
 begin
   map(senses,proc(x) // Only equality contraints are allowed in symbolic solve blocks.
               begin  
                 if x <> "E" then error(errorInvalidSolveConstraint)
                 end_if;
               end_proc);
   unknowns := [op(constraints[1],1)];

   eqns := unknowns -> map(constraints, c->( c(op(unknowns))=0 ));
...
basefuncs::op_solve_internal := proc(eqns_func, unknowns, is_minerr = FALSE)
 local subsin, subsout, rawres, res, u, v;
 begin
   //print("op_solve_internal","eqns_func=",eqns_func,"unknowns=",unknowns);
   ...
   if is_minerr then
      rawres := mcMinerr(eqns,unknowns);
   else
      rawres := mcSolve(eqns,unknowns);
   end_if;

   if (rawres = FAIL) then
      if optionSolveTryNumeric then
         rawres := evalFloat(hold(solve)(eqn,vars));
      end_if;
...
basefuncs::symsubscript1 := proc(M,i,origin)
 begin
   ...
   if domtype(M) = DOM_PROC then
      ...
      error(errorNotMatrix);
   elif isMatrix(i) or isString(i) or
      ( isScalar(i) and not isInteger(i) ) then
      error(errorNotInteger);
   else
 ...
 begin
   ...
   if domtype(M) = DOM_PROC then
      ...
      error(errorNotMatrix);
        ...
        ( isScalar(i) and not isInteger(i) ) or
        isMatrix(j) or isString(j) or 
        ( isScalar(j) and not isInteger(j) ) then
      error(errorNotInteger);
   else
